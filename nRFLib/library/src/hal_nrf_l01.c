...
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
#define SET_BIT_IN_BYTE(pos) ((uint8_t) (1<<( (uint8_t) (pos) )))
...
void hal_nrf_set_irq_mode(hal_nrf_irq_source_t int_source, bool irq_state)
{
  if(irq_state)
  {
    hal_nrf_write_reg(CONFIG, hal_nrf_read_reg(CONFIG) & ~SET_BIT_IN_BYTE(int_source));
  }
  else
  {
    hal_nrf_write_reg(CONFIG, hal_nrf_read_reg(CONFIG) | SET_BIT_IN_BYTE(int_source));
...
{
  hal_nrf_write_reg(STATUS, SET_BIT_IN_BYTE(int_source));
}
 
bool hal_nrf_get_irq_mode(uint8_t int_type)
{
  if(hal_nrf_read_reg(CONFIG) & SET_BIT_IN_BYTE(int_type))
    return FALSE;
  else
    return TRUE;
}
...
{
  hal_nrf_write_reg(CONFIG, (hal_nrf_read_reg(CONFIG) & ~(BIT_3|BIT_2)) | ((uint8_t)(crc_mode)<<2));
}
...
void hal_nrf_open_pipe(hal_nrf_address_t pipe_num, bool auto_ack)
{
  switch(pipe_num)
  {
    ...
    case HAL_NRF_PIPE4:
    case HAL_NRF_PIPE5:
      hal_nrf_write_reg(EN_RXADDR, hal_nrf_read_reg(EN_RXADDR) | SET_BIT_IN_BYTE(pipe_num));

      if(auto_ack)
        hal_nrf_write_reg(EN_AA, hal_nrf_read_reg(EN_AA) | SET_BIT_IN_BYTE(pipe_num));
      else
        hal_nrf_write_reg(EN_AA, hal_nrf_read_reg(EN_AA) & ~SET_BIT_IN_BYTE(pipe_num));
      break;
...
{
  hal_nrf_write_reg(RF_SETUP, (hal_nrf_read_reg(RF_SETUP) & ~((1<<RF_PWR1)|(1<<RF_PWR0))) | ((uint8_t)(power)<<1));
}
...
uint8_t hal_nrf_write_reg(uint8_t reg, uint8_t value)
{
  uint8_t retval;
  CSN_LOW();
  if(reg < REG_WRITE)   // i.e. this is a register access
  {
    retval = hal_nrf_rw(REG_WRITE + reg);
    hal_nrf_rw(value);
  }
...
void hal_nrf_write_multibyte_reg(uint8_t reg, uint8_t *pbuf, uint8_t length)
{
  switch(reg)
  {
    ...
    case HAL_NRF_TX:
      ...
      CSN_LOW();
      hal_nrf_rw(REG_WRITE + RX_ADDR_P0 + reg);
      break;
